function calendarFun() { 
    const calendar = { 
        ...
        nStr2: ["\u521d", "\u5341", "\u5eff", "\u5345"], nStr3: ["\u6b63", "\u4e8c", "\u4e09", "\u56db", "\u4e94", "\u516d", "\u4e03", "\u516b", "\u4e5d", "\u5341", "\u51ac", "\u814a",], 
        lYearDays: function (y) { 
            var i, sum = 348; for (i = 0x8000; i > 0x8; i >>= 1) { sum += calendar.lunarInfo[y - 1900] & i ? 1 : 0 } return sum + calendar.leapDays(y) 
        }, 
        ...
        solarDays: function (y, m) { 
            ...
            var ms = m - 1; 
            if (ms == 1) { return (y % 4 == 0 && y % 100 != 0) || y % 400 == 0 ? 29 : 28 
            } else { 
        ...
        toChinaDay: function (d) { 
            var s; switch (d) { 
                ...
                break; 
                break; 
                default: s = calendar.nStr2[Math.floor(d / 10)]; 
        ...
        solar2lunar: function (y, m, d) { 
            ...
            for (i = 1; i < 13 && offset > 0; i++) { 
                if (leap > 0 && i == leap + 1 && isLeap == false) { 
                    --i; isLeap = true; temp = calendar.leapDays(year) 
                } else { 
            ...
            return { 
                lYear: year, lMonth: month, lDay: day, Animal: calendar.getAnimal(year), IMonthCn: (isLeap ? "\u95f0" : "") + calendar.toChinaMonth(month), IDayCn: calendar.toChinaDay(day), cYear: y, cMonth: m, cDay: d, gzYear: gzY, gzMonth: gzM, gzDay: gzD, isToday: isToday, isLeap: isLeap, nWeek: nWeek, ncWeek: "\u661f\u671f" + cWeek, isTerm: isTerm, Term: Term, astro: astro, 
            } 
        }, 
        lunar2solar: function (y, m, d, isLeapMonth) { 
            ...
            var _day = day; 
            if (isLeapMonth) { _day = calendar.leapDays(y, m) } 
            ...
            for (var i = 1; i < m; i++) { 
                ...
                if (!isAdd) { 
                    if (leap <= i && leap > 0) { offset += calendar.leapDays(y); 
                        isAdd = true 
                    } 
                } 
            ...
            var calObj = new Date((offset + d - 31) * 86400000 + stmap); var cY = calObj.getUTCFullYear(); 
            var cM = calObj.getUTCMonth() + 1; 
            var cD = calObj.getUTCDate(); 
            return calendar.solar2lunar(cY, cM, cD) 
        }, 
        birthday: function (y, m, d, nongli, isLeapMonth) { 
            var date; 
            var now = new Date(); 
            if (nongli) { 
                var now_d = this.solar2lunar(now.getFullYear(), now.getMonth() + 1, now.getDate()); 
            ...
            return [date.cYear, date.cMonth, date.cDay, this.daysBetween(date)] 
        }, 
        birthBylunar: function (y, m, d, isLeapMonth) { 
        ...
        getAstroToEmoji(astro) { 
            var data = { 白羊座: "♈", 金牛座: "♉", 双子座: "♊", 巨蟹座: "♋", 狮子座: "♌", 处女座: "♍", 天秤座: "♎", 天蝎座: "♏", 射手座: "♐", 摩羯座: "♑", 水瓶座: "♒", 双鱼座: "♓", 蛇夫座: "⛎", }; 
            return data[astro] || "" 
        }, 
    }; 
    return calendar 
